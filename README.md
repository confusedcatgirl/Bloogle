...
`indexer/` = Indexes all collected data.
...
Inside of the pages folder, a certain set of policies have to be met.

First, all domains have their own folder. Subfolders are subdomains, root is simply named: \_\_root\_\_

A domain named `cloud.confusedblorb.org` would result in a path `./data/pages/confusedblorb.org/cloud`, the root domain would end up as `.data/pages/confusedblorb.org/__root__`

The data is placed in each folder accordingly.
...
│       ├── Rank            Int     Not Null
│       └── page_ID         Int     Foreign Key, Not Null
│
├── repo
│   ├── queue
│   │   ├── ID              Int     Primary Key, Not Null
